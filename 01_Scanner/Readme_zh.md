...
在最后，你发现我们调用了 `putback(c)` 了吗？我们遇到了一个非十进制数字，我们不能简单地丢弃它，不过好在我们能把它放回输入流以便之后使用。
...
在我们编译器旅程的下一部分中，我们会构建一个递归下降(recursive descent)解析器来解析输入文件的语法，计算并输出每个文件的最终值。[下一步](../02_Parser/Readme_zh.md)
